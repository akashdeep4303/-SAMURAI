...
model_cfg = f"configs/samurai/sam2.1_hiera_{model_name}.yaml" if model_name != "base_plus" else "configs/samurai/sam2.1_hiera_b+.yaml"
...
for vid, video in enumerate(test_videos):
    cat_name, cid_name, video_basename = video.split('-')[0], video.split('-')[1], video.strip()
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
        ...
        prompts = load_lasot_gt(osp.join(video_folder, cat_name, video_basename, "groundtruth.txt"))
